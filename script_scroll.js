...
// 7. Stack-in Roles with smooth staggered scroll
const roleTimeline = gsap.timeline({
  scrollTrigger: {
    trigger: ".roles-stack",
    start: "top top",
    end: "+=2000", // Ajusta longitud del scroll
    scrub: true,
    pin: true,
  }
});

// Animar cada línea en orden con separación
document.querySelectorAll(".role-line").forEach((line, i) => {
  roleTimeline.to(line, {
    opacity: 1,
    y: 0,
    duration: 1,
    ease: "power3.out"
  }, i * 0.8); // espacio entre una línea y otra (0.8s)
});
